...
export function renderWithAuth(ui: ReactElement, options?: RenderWithAuthOptions) {
  ...
  const tokenData = {token: initialToken, createdAt: Date.now(), expiresAt: Date.now() + 3600000}
 ...
 */
export function mockFetchError(status: number = 500, message: string = 'Internal Server Error'): Mock {
  const fetchMock = vi.fn(async () => {
 ...
 */
export function setupOAuthCallback(accessToken: string, expiresIn: number = 3600): void {
 ...
 */
export function mockAuthFlow(token: string = 'test_token'): void {
  const tokenData = {token, createdAt: Date.now(), expiresAt: Date.now() + 3600000}
  localStorage.setItem('spotify_token_data', JSON.stringify(tokenData))
  setupSpotifyMocks()
}
