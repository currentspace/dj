 ...
 */
export const ErrorDisplay = memo(function ErrorDisplay({error, onDismiss, variant = 'inline'}: ErrorDisplayProps) {
  ...
  return (
    <div aria-live="polite" className={className} role="alert">
      <div className="error-display__content">
        <span aria-hidden="true" className="error-display__icon">
          !
        </span>
        <span className="error-display__message">{error}</span>
      </div>
      {onDismiss && (
        <button aria-label="Dismiss error" className="error-display__dismiss" onClick={onDismiss} type="button">
