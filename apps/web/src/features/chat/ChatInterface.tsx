...
export function ChatInterface({ selectedPlaylist }: ChatInterfaceProps) {
  ...
  >(new Map());
  const [currentPlaylistId, setCurrentPlaylistId] = useState<null | string>(
    null
  );
  ...
  if (playlistId !== currentPlaylistId) {
    console.log(
      `[ChatInterface] Switching playlist context: ${currentPlaylistId} → ${playlistId}`
    );
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      console.log(
        `[ChatInterface] Pre-injection - Mode: ${mode}, SelectedID: ${playlistId}`
      );
      if ((mode === "analyze" || mode === "edit") && playlistId) {
        ...
        console.warn(
          `[ChatInterface] No playlist ID injected - Mode: ${mode}, ID: ${playlistId}`
        );
      }
      ...
      flushSync(() => {
        setConversationsByPlaylist((prev) => {
          ...
          newMap.set(playlistKey, [
            ...currentMessages,
            { content: displayMessage, role: "user" },
          ]);
          return newMap;
        });
      });
    ...
    return (
      <div className="chat-interface">
        <div className="no-playlist-selected">
          ...
          <p>
            Choose a playlist from the left to start chatting with your AI DJ
            assistant!
          </p>
          <p>
            I can help you analyze your music, create new playlists, or edit
            existing ones.
          </p>
        </div>
      </div>
  ...
  return (
    <div className="chat-interface">
        ...
        <div className="mode-selector">
          ...
          {(mode === "analyze" || mode === "edit") && (
            <div className="selected-playlist-info">
              <span>
                🎵 {selectedPlaylist.name} ({selectedPlaylist.tracks.total}{" "}
                tracks)
              </span>
            </div>
          )}
      ...
      <form className="chat-input-form" onSubmit={handleSubmit}>
        ...
        />
        <button
          disabled={streamingStatus.isStreaming || !input.trim()}
          type="submit"
        >
