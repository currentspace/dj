...
app.post('/spotify', async c => {
  ...
  const isValid = await verifySpotifyWebhook(body, signature, timestamp, c.env.SPOTIFY_WEBHOOK_SECRET)
...
async function processSpotifyWebhook(payload: SpotifyWebhookPayload, env: Env): Promise<void> {
