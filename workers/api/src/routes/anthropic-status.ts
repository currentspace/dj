...
anthropicStatusRouter.get("/limits", async (c) => {
  ...
  console.log(
    `[AnthropicStatus:${requestId}] Checking rate limits and account status`
  );
  ...
  try {
    ...
    const rateLimitHeaders = {
      "requests-limit": response.headers.get(
        "anthropic-ratelimit-requests-limit"
      ),
      "requests-remaining": response.headers.get(
        "anthropic-ratelimit-requests-remaining"
      ),
      "requests-reset": response.headers.get(
        "anthropic-ratelimit-requests-reset"
      ),
      "retry-after": response.headers.get("retry-after"),
      "tokens-limit": response.headers.get("anthropic-ratelimit-tokens-limit"),
      "tokens-remaining": response.headers.get(
    ...
    console.log(
      `[AnthropicStatus:${requestId}] Rate limit headers:`,
      rateLimitHeaders
    );
    ...
    if (response.status === 200) {
      ...
      details = {
        ...rateLimitHeaders,
        requestsUsage:
          rateLimitHeaders["requests-remaining"] &&
          rateLimitHeaders["requests-limit"]
            ? `${
                rateLimitHeaders["requests-limit"] -
                rateLimitHeaders["requests-remaining"]
              }/${rateLimitHeaders["requests-limit"]}`
    ...
    console.error(
      `[AnthropicStatus:${requestId}] Error checking status:`,
      error
    );
    return c.json(
      {
...
anthropicStatusRouter.get("/usage", async (c) => {
  ...
  try {
    ...
    const requestsReset = response.headers.get(
      "anthropic-ratelimit-requests-reset"
    );
    ...
    const usage = {
      ...
      requests: {
        ...
        resetsIn: requestsReset
          ? `${Math.round(
              (new Date(requestsReset).getTime() - now.getTime()) / 1000
            )} seconds`
          : null,
        used: requestsUsed,
      },
      ...
      tokens: {
        ...
        resetsIn: tokensReset
          ? `${Math.round(
              (new Date(tokensReset).getTime() - now.getTime()) / 1000
            )} seconds`
          : null,
        used: tokensUsed,
      },
    ...
    console.error(
      `[AnthropicStatus:${requestId}] Error checking usage:`,
      error
    );
    return c.json(
      {
