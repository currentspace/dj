  ...
  try {
    ...
    if (response.status === 200) {
      ...
      details = {
        ...rateLimitHeaders,
        requestsUsage:
          rateLimitHeaders['requests-remaining'] && rateLimitHeaders['requests-limit']
            ? `${
                rateLimitHeaders['requests-limit'] - rateLimitHeaders['requests-remaining']
    ...
    const usage = {
      ...
      requests: {
        ...
        resetsIn: requestsReset
          ? `${Math.round((new Date(requestsReset).getTime() - now.getTime()) / 1000)} seconds`
          : null,
        used: requestsUsed,
      },
      ...
      tokens: {
        ...
        resetsIn: tokensReset
          ? `${Math.round((new Date(tokensReset).getTime() - now.getTime()) / 1000)} seconds`
          : null,
        used: tokensUsed,
      },
      warnings: [],
