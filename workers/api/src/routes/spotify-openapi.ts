...
app.openapi(handleSpotifyCallback, async (c) => {
  ...
  try {
    ...
    if (!cookieValue) {
      ...
      return c.redirect(
        `${env.FRONTEND_URL ?? 'https://dj.current.space'}?error=missing_cookie`
      );
    }
    ...
    if (!payload || !signature) {
      ...
      return c.redirect(
        `${env.FRONTEND_URL ?? 'https://dj.current.space'}?error=invalid_cookie`
      );
    }
    ...
    if (!isValidSignature) {
      ...
      return c.redirect(
        `${env.FRONTEND_URL ?? 'https://dj.current.space'}?error=tampered_cookie`
      );
    }
    ...
    if (state !== cookieState) {
      ...
      return c.redirect(
        `${env.FRONTEND_URL ?? 'https://dj.current.space'}?error=state_mismatch`
      );
    }
    ...
    if (Date.now() - timestamp > maxAge) {
      ...
      return c.redirect(
        `${env.FRONTEND_URL ?? 'https://dj.current.space'}?error=expired_auth`
      );
    }
    ...
    return c.redirect(
      `${env.FRONTEND_URL ?? 'https://dj.current.space'}?error=callback_failed`
    );
  }
});
