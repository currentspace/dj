...
Use tools to make informed decisions.`,
    ...
    if (initialResponse?.tool_calls && initialResponse.tool_calls.length > 0) {
      ...
      for (let i = 0; i < initialResponse.tool_calls.length; i++) {
        ...
        if (!tool) {
          console.error(
            `[Chat:${requestId}] ❌ Tool '${toolCall.name}' not found in available tools: [${tools
              .map(t => t.name)
              .join(', ')}]`,
          )
          toolResults.push({
        ...
        try {
          ...
          if (error instanceof Error && error.stack) {
            console.error(
              `[Chat:${requestId}] [Tool ${i + 1}/${initialResponse.tool_calls.length}] Stack:`,
              error.stack.split('\n').slice(0, 3).join('\n'),
            )
          }
          toolResults.push({
      ...
      finalResponse = await llm.invoke([...messages, initialResponse, toolResponseMessage])
      ...
      console.log(
        `[Chat:${requestId}] Final response length: ${
          typeof finalResponse?.content === 'string' ? finalResponse.content.length : 'N/A'
        } chars`,
      )
    }
