  ...
  const tools: DynamicStructuredTool[] = [
    ...
    new DynamicStructuredTool({
      ...
      func: async (args) => {
        ...
        try {
          const result = await executeSpotifyTool('get_track_details', args, spotifyToken);
          const duration = Date.now() - startTime;
          console.log(`[Tool:get_track_details] Success: Retrieved track details in ${duration}ms`);
          return result;
        } catch (error) {
          const duration = Date.now() - startTime;
          console.error(`[Tool:get_track_details] Failed after ${duration}ms:`, error instanceof Error ? error.message : error);
          throw error;
        }
      }
    }),
    ...
    new DynamicStructuredTool({
      ...
      func: async (args) => {
        ...
        try {
          const result = await executeSpotifyTool('get_artist_info', args, spotifyToken);
          const duration = Date.now() - startTime;
          console.log(`[Tool:get_artist_info] Success: Retrieved artist info in ${duration}ms`);
          return result;
        } catch (error) {
          const duration = Date.now() - startTime;
          console.error(`[Tool:get_artist_info] Failed after ${duration}ms:`, error instanceof Error ? error.message : error);
          throw error;
        }
      }
    }),
    ...
    new DynamicStructuredTool({
      ...
      schema: z.object({
        artist_id: z.string().describe('Spotify artist ID'),
        market: z.string().default('US').describe('Market/country code for track availability')
      }),
      func: async (args) => {
        const startTime = Date.now();
        console.log(`[Tool:get_artist_top_tracks] Getting top tracks for artist ${args.artist_id}`);
        try {
          const result = await executeSpotifyTool('get_artist_top_tracks', args, spotifyToken);
          const duration = Date.now() - startTime;
          console.log(`[Tool:get_artist_top_tracks] Success: Retrieved ${Array.isArray(result) ? result.length : 0} top tracks in ${duration}ms`);
          return result;
        } catch (error) {
          const duration = Date.now() - startTime;
          console.error(`[Tool:get_artist_top_tracks] Failed after ${duration}ms:`, error instanceof Error ? error.message : error);
          throw error;
        }
      }
    }),
    ...
    new DynamicStructuredTool({
      ...
      func: async (args) => {
        const startTime = Date.now();
        console.log(`[Tool:search_artists] Searching for artists: "${args.query}"`);
        try {
          const result = await executeSpotifyTool('search_artists', args, spotifyToken);
          const duration = Date.now() - startTime;
          console.log(`[Tool:search_artists] Success: Found ${Array.isArray(result) ? result.length : 0} artists in ${duration}ms`);
          return result;
        } catch (error) {
          const duration = Date.now() - startTime;
          console.error(`[Tool:search_artists] Failed after ${duration}ms:`, error instanceof Error ? error.message : error);
          throw error;
        }
      }
    }),
    ...
    new DynamicStructuredTool({
      ...
      schema: z.object({
        artist_id: z.string().describe('Spotify artist ID to find related artists for')
      }),
      func: async (args) => {
        const startTime = Date.now();
        console.log(`[Tool:get_related_artists] Getting related artists for ${args.artist_id}`);
        try {
          const result = await executeSpotifyTool('get_related_artists', args, spotifyToken);
          const duration = Date.now() - startTime;
          console.log(`[Tool:get_related_artists] Success: Found ${Array.isArray(result) ? result.length : 0} related artists in ${duration}ms`);
          return result;
        } catch (error) {
          const duration = Date.now() - startTime;
          console.error(`[Tool:get_related_artists] Failed after ${duration}ms:`, error instanceof Error ? error.message : error);
          throw error;
        }
      }
    }),
    ...
    new DynamicStructuredTool({
      name: 'get_album_info',
      description: 'Get detailed information about an album including tracks and release date',
      schema: z.object({
        album_id: z.string().describe('Spotify album ID')
      }),
      func: async (args) => {
        const startTime = Date.now();
        console.log(`[Tool:get_album_info] Getting info for album ${args.album_id}`);
        try {
          const result = await executeSpotifyTool('get_album_info', args, spotifyToken);
          const duration = Date.now() - startTime;
          console.log(`[Tool:get_album_info] Success: Retrieved album info in ${duration}ms`);
          return result;
        } catch (error) {
          const duration = Date.now() - startTime;
          console.error(`[Tool:get_album_info] Failed after ${duration}ms:`, error instanceof Error ? error.message : error);
          throw error;
        }
      }
    }),
    ...
    new DynamicStructuredTool({
      ...
      func: async () => {
        const startTime = Date.now();
        console.log(`[Tool:get_available_genres] Getting available genre seeds`);
        try {
          const result = await executeSpotifyTool('get_available_genres', {}, spotifyToken);
          const duration = Date.now() - startTime;
          console.log(`[Tool:get_available_genres] Success: Retrieved ${Array.isArray(result) ? result.length : 0} genres in ${duration}ms`);
          return result;
        } catch (error) {
          const duration = Date.now() - startTime;
          console.error(`[Tool:get_available_genres] Failed after ${duration}ms:`, error instanceof Error ? error.message : error);
          throw error;
        }
      }
    })
