  ...
  app.openapi(getUserPlaylists, async (c) => {
    try {
      ...
      if (!isSuccessResponse(response)) {
        console.error(
          `Get playlists failed: ${response.status} ${response.statusText}`
        );
  ...
  app.openapi(getPlaylistTracks, async (c) => {
    try {
      ...
      if (!isSuccessResponse(response)) {
        ...
        console.error(
          `Get playlist tracks failed: ${response.status} ${response.statusText}`
        );
      ...
      const rawData = await response.json();
      const parseResult =
        SpotifyPlaylistTracksResponseSchema.safeParse(rawData);

      if (!parseResult.success) {
        console.error(
          "Invalid Spotify playlist tracks response:",
          parseResult.error
        );
        return c.json({ error: "Invalid response from Spotify API" }, 500);
      }
  ...
  app.openapi(createPlaylist, async (c) => {
    ...
    try {
      ...
      const playlistParseResult =
        SpotifyPlaylistFullSchema.safeParse(rawPlaylist);

      if (!playlistParseResult.success) {
        console.error(
          "Invalid Spotify create playlist response:",
  ...
  app.openapi(modifyPlaylist, async (c) => {
    try {
      ...
      if (action === "add") {
        ...
        if (!isSuccessResponse(response)) {
          console.error(
            `Add tracks failed: ${response.status} ${response.statusText}`
          );
          return c.json({ error: "Failed to add tracks" }, 400);
        }

        const rawResult = await response.json();
        const addParseResult =
          SpotifyAddTracksResponseSchema.safeParse(rawResult);

        if (!addParseResult.success) {
          console.error(
            "Invalid Spotify add tracks response:",
            addParseResult.error
          );
          return c.json({ error: "Invalid response from Spotify API" }, 500);
        }
        ...
        if (!isSuccessResponse(response)) {
          console.error(
            `Remove tracks failed: ${response.status} ${response.statusText}`
          );
          return c.json({ error: "Failed to remove tracks" }, 400);
        }

        const rawResult = await response.json();
        const removeParseResult =
          SpotifyAddTracksResponseSchema.safeParse(rawResult);

        if (!removeParseResult.success) {
          console.error(
            "Invalid Spotify remove tracks response:",
            removeParseResult.error
          );
          return c.json({ error: "Invalid response from Spotify API" }, 500);
        }
      ...
      const message =
        error instanceof Error ? error.message : "Failed to modify playlist";
      return c.json({ error: message }, 400);
    }
  });
}
