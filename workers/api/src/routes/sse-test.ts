...
sseTestRouter.get("/simple", async () => {
  ...
  const processStream = async () => {
    try {
      ...
      await writer.write(
        encoder.encode('data: {"type": "start", "data": "Starting test"}\n\n')
      );
      ...
      for (let i = 0; i < 5; i++) {
        ...
        await writer.write(
          encoder.encode(
            `data: {"type": "message", "data": "Message ${i + 1}"}\n\n`
          )
        );
      }
...
sseTestRouter.post("/post-stream", async (c) => {
  ...
  const processStream = async () => {
    try {
      ...
      await writer.write(
        encoder.encode(
          `data: {"type": "echo", "data": ${JSON.stringify(body)}}\n\n`
        )
      );
      ...
      await writer.write(
        encoder.encode(`data: {"type": "error", "data": "${errorMessage}"}\n\n`)
      );
    } finally {
      await writer.close();
    }
