...
mcpRouter.all("/", async (c) => {
  ...
  try {
    ...
    console.log(
      `[MCP:${requestId}] Session-Id: ${sessionId?.substring(0, 8) ?? "none"}`
    );

    // Helper functions for safe header handling
    const getHeader = (name: string) =>
      c.req.header(name) ?? c.req.raw.headers.get(name);
    ...
    console.log(
      `[MCP:${requestId}] Authorization header present: ${!!authorization}`
    );
    console.log(
      `[MCP:${requestId}] Authorization format correct: ${authorization?.startsWith(
        "Bearer "
      )}`
    );
    ...
    if (!authorization?.startsWith("Bearer ")) {
      ...
      console.warn(
        `[MCP:${requestId}] UNAUTHORIZED - No bearer token provided (${duration}ms)`
      );
      console.log(
        `[MCP:${requestId}] Authorization header value: ${
    ...
    console.log(
      `[MCP:${requestId}] Validating session with session manager...`
    );
    ...
    try {
      ...
      console.log(
        `[MCP:${requestId}] Session validation completed, token found: ${!!spotifyToken}`
      );
    } catch (sessionError) {
      const duration = Date.now() - startTime;
      console.error(
    ...
    if (!spotifyToken) {
      ...
      console.log(
        `[MCP:${requestId}] Session validation failed for: ${sessionToken.substring(
          0,
          8
        )}...`
      );
    ...
    console.log(
      `[MCP:${requestId}] Spotify token: ${spotifyToken.substring(0, 20)}...`
    );
    ...
    if (method === "GET") {
      ...
      if (!acceptHeader.includes("text/event-stream")) {
        ...
        console.log(
          `[MCP:${requestId}] Expected: text/event-stream, Got: ${acceptHeader}`
        );
      ...
      const duration = Date.now() - startTime;
      console.log(
        `[MCP:${requestId}] Initializing Worker-safe SSE stream (${duration}ms)`
      );
      ...
      const stream = new ReadableStream({
        start(controller) {
          ...
          console.log(
            `[MCP:${requestId}] SSE connection established, events sent`
          );
          ...
          const heartbeat = setInterval(() => {
            try {
              ...
              console.log(
                `[MCP:${requestId}] Heartbeat failed, client likely disconnected`
              );
    ...
    if (method === "POST") {
      ...
      if (!acceptHeader.includes("application/json")) {
        ...
        console.log(
          `[MCP:${requestId}] Expected: application/json, Got: ${acceptHeader}`
        );
      ...
      try {
        ...
        console.log(
          `[MCP:${requestId}] Request body (${bodyStr.length} chars):`,
          bodyStr.substring(0, 300) + (bodyStr.length > 300 ? "..." : "")
        );
        ...
        for (let i = 0; i < requests.length; i++) {
          ...
          try {
            ...
            console.log(
              `[MCP:${requestId}] Request ${i + 1} completed successfully`
            );
          } catch (error) {
            console.error(
              `[MCP:${requestId}] Request ${i + 1} validation error:`,
              error
            );
            console.log(
        ...
        if (responses.length === 0) {
          ...
          console.log(
            `[MCP:${requestId}] All requests were notifications, returning 204`
          );
        ...
        console.log(
          `[MCP:${requestId}] === REQUEST COMPLETE (${duration}ms) ===`
        );
        console.log(
          `[MCP:${requestId}] Processed: ${requests.length} request(s)`
        );
        console.log(
          `[MCP:${requestId}] Response size: ${resultStr.length} chars`
        );
        console.log(
          `[MCP:${requestId}] Response preview:`,
        ...
        const duration = Date.now() - startTime;
        console.error(
          `[MCP:${requestId}] POST FATAL ERROR after ${duration}ms:`,
          error
        );
    ...
    const duration = Date.now() - startTime;
    console.error(
      `[MCP:${requestId}] UNSUPPORTED METHOD: ${method} (${duration}ms)`
    );
    ...
    if (error instanceof Error) {
      ...
      console.error(
        `[MCP:${requestId}] Error stack: ${error.stack?.substring(0, 1000)}`
      );
    } else {
...
async function handleMCPRequest(
  ...
  switch (method) {
    case "initialize": {
      ...
      console.log(
        `[MCP:${requestId}] INITIALIZE completed in ${initDuration}ms`
      );
      console.log(
        `[MCP:${requestId}] Protocol version: ${initResult.result.protocolVersion}`
      );
      console.log(
        `[MCP:${requestId}] Server: ${initResult.result.serverInfo.name} v${initResult.result.serverInfo.version}`
      );
    ...
    case "tools/list": {
      ...
      console.log(
        `[MCP:${requestId}] TOOLS/LIST completed in ${listDuration}ms`
      );
      console.log(
        `[MCP:${requestId}] Available tools: ${toolsResult.result.tools.length}`
      );
      console.log(
        `[MCP:${requestId}] Tool names: [${toolsResult.result.tools
          .map((t) => t.name)
          .join(", ")}]`
      );
    ...
    case "tools/call": {
      ...
      console.log(
        `[MCP:${requestId}] Tool arguments:`,
        JSON.stringify(args).substring(0, 200)
      );
      console.log(
        `[MCP:${requestId}] Spotify token available: ${!!spotifyToken}`
      );

      if (!name) {
        console.error(
          `[MCP:${requestId}] TOOLS/CALL ERROR - No tool name provided`
        );
        return {
          error: {
            code: -32602,
      ...
      try {
        ...
        console.log(
          `[MCP:${requestId}] TOOLS/CALL SUCCESS - ${name} completed`
        );
        console.log(
          `[MCP:${requestId}] Tool execution time: ${toolDuration}ms`
        );
        console.log(`[MCP:${requestId}] Total method time: ${totalDuration}ms`);
        console.log(
          `[MCP:${requestId}] Result size: ${resultStr.length} chars`
        );
        console.log(
          `[MCP:${requestId}] Result preview:`,
          resultStr.substring(0, 200) + (resultStr.length > 200 ? "..." : "")
        );
        ...
        console.log(`[MCP:${requestId}] Tool error details:`, {
          ...
          stack:
            toolError instanceof Error
              ? toolError.stack?.substring(0, 300)
              : undefined,
        });
    ...
    default: {
      ...
      console.error(
        `[MCP:${requestId}] UNKNOWN METHOD: ${method} (${duration}ms)`
      );
      console.log(
        `[MCP:${requestId}] Available methods: initialize, tools/list, tools/call`
      );
      console.log(
        `[MCP:${requestId}] Request data:`,
        JSON.stringify(request).substring(0, 200)
      );
...
mcpRouter.post("/session/create", async (c) => {
  ...
  try {
    ...
    console.log(
      `[MCP:${sessionRequestId}] Authorization header present: ${!!authorization}`
    );
    console.log(
      `[MCP:${sessionRequestId}] Spotify token length: ${
    ...
    console.log(
      `[MCP:${sessionRequestId}] Spotify token: ${spotifyToken.substring(
        0,
        20
      )}...`
    );
    ...
    if (!testResponse.ok) {
      ...
      console.error(
        `[MCP:${sessionRequestId}] SESSION CREATE FAILED - Invalid Spotify token`
      );
      console.log(
        `[MCP:${sessionRequestId}] Status: ${testResponse.status}, Duration: ${duration}ms`
      );
      console.log(
        `[MCP:${sessionRequestId}] Spotify error: ${errorText.substring(
          0,
          200
        )}`
      );
    ...
    console.log(
      `[MCP:${sessionRequestId}] Spotify user validated: ${userData.id}`
    );
    console.log(
      `[MCP:${sessionRequestId}] Display name: ${
        userData.display_name ?? "none"
      }`
    );
    console.log(
      `[MCP:${sessionRequestId}] Country: ${userData.country ?? "unknown"}`
    );
    console.log(
      `[MCP:${sessionRequestId}] Followers: ${userData.followers?.total ?? 0}`
    );
    ...
    console.log(
      `[MCP:${sessionRequestId}] Session creation time: ${sessionCreationTime}ms`
    );
    ...
    const duration = Date.now() - startTime;
    console.error(
      `[MCP:${sessionRequestId}] SESSION CREATE ERROR after ${duration}ms:`,
      error
    );
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      const send = (data: unknown, event?: string) => {
        ...
        controller.enqueue(enc.encode(message));
        console.log(
          `[SSE-Test:${testId}] Sent: ${event ?? "data"} - ${JSON.stringify(
            data
          ).substring(0, 100)}`
        );
      };
      ...
      const interval = setInterval(() => {
        ...
        try {
          ...
          if (counter >= 5) {
            ...
            controller.close();
            console.log(
              `[SSE-Test:${testId}] Test completed, connection closed`
            );
          }
        } catch (error) {
...
mcpRouter.post("/session/destroy", async (c) => {
  ...
  try {
    ...
    console.log(
      `[MCP:${destroyRequestId}] === SESSION DESTROY SUCCESS (${duration}ms) ===`
    );
    ...
    const duration = Date.now() - startTime;
    console.error(
      `[MCP:${destroyRequestId}] SESSION DESTROY ERROR after ${duration}ms:`,
      error
    );
