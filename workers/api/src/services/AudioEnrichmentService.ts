...
export class AudioEnrichmentService {
  ...
  private async deezerSingleByISRC(isrc: string): Promise<DeezerTrack | null> {
    try {
      ...
      console.log(
        `[BPMEnrichment] Deezer direct ISRC response status: ${response?.status ?? 'null'}`,
      )
      if (!response?.ok) {
        return null
      }
  ...
  private async deezerTrackById(id: number): Promise<DeezerTrack | null> {
    try {
      ...
      console.log(
        `[BPMEnrichment] Deezer track by ID response status: ${response?.status ?? 'null'}`,
      )
      if (!response?.ok) {
        return null
      }
  ...
  private async setCached(
    ...
    try {
      ...
      console.log(
        `[BPMEnrichment] Cached ${isMiss ? 'miss' : 'hit'} for ${trackId} (TTL: ${ttlDisplay})`,
      )
    } catch (error) {
