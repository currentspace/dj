...
const DEFAULT_ALLOW_RULES: ExternalRule[] = [
  ...
  {
    id: 'allow-pnpm-safe',
    pattern: '^pnpm\\s+(install|dev|build|lint|test|run|typecheck)\\b',
    reason: 'Safe pnpm dev operations',
    type: 'regex',
  },
...
const DEFAULT_DENY_RULES: ExternalRule[] = [
  // Package manager enforcement - use pnpm/uv instead
  {
    id: 'deny-npx',
    pattern: 'npx ',
    reason: "Project uses pnpm - use 'pnpm exec' or 'pnpm dlx' instead of npx",
    type: 'prefix',
  },
  {
    id: 'deny-npm-install',
    pattern: '^npm\\s+(install|i|add|ci)\\b',
    reason: "Project uses pnpm - use 'pnpm install' or 'pnpm add' instead",
    type: 'regex',
  },
  {
    id: 'deny-yarn',
    pattern: '^yarn\\s+(install|add)\\b',
    reason: "Project uses pnpm - use 'pnpm install' or 'pnpm add' instead",
    type: 'regex',
  },
  ...
  {
    id: 'deny-rm-rf-root',
    pattern: 'rm\\s+(-[a-zA-Z]*r[a-zA-Z]*f|\\-[a-zA-Z]*f[a-zA-Z]*r)[a-zA-Z]*\\s+/(?!Users|home|tmp)',
    reason: 'Destructive: removes root filesystem paths',
    type: 'regex',
  },
  {
    id: 'deny-rm-rf-slash',
    pattern: 'rm\\s+(-[a-zA-Z]*r[a-zA-Z]*|--recursive)(?:\\s+\\S+)*\\s+/\\s*$',
    reason: 'Destructive: removes root filesystem',
    type: 'regex',
  },
  {
    id: 'deny-rm-rf-home',
    pattern: 'rm\\s+(-[a-zA-Z]*r[a-zA-Z]*|--recursive)(?:\\s+\\S+)*\\s+~/?\\s*$',
    reason: 'Destructive: removes entire home directory',
    type: 'regex',
  },
  {
    id: 'deny-rm-rf-star',
    pattern: 'rm\\s+(-[a-zA-Z]*r[a-zA-Z]*|--recursive)(?:\\s+\\S+)*\\s+/\\*',
    reason: 'Destructive: removes all root level directories',
    type: 'regex',
  },
  {
    id: 'deny-rm-system-dirs',
    pattern: 'rm\\s+(-[a-zA-Z]*r[a-zA-Z]*|--recursive)(?:\\s+\\S+)*\\s+/(etc|var|usr|bin|sbin|lib|boot|sys|proc)\\b',
    reason: 'Destructive: removes system directories',
    type: 'regex',
  },
  {
    id: 'deny-rm-no-preserve-root',
    pattern: '--no-preserve-root',
    reason: 'Destructive: bypasses rm safety',
    type: 'contains',
  },
  ...
  {
    id: 'deny-chmod-777',
    pattern: 'chmod\\s+(-[a-zA-Z]+\\s+)*777',
    reason: 'Insecure permissions: world-writable',
    type: 'regex',
  },
  {
    id: 'deny-chmod-666',
    pattern: 'chmod\\s+(-[a-zA-Z]+\\s+)*666',
    reason: 'Insecure permissions: world-writable files',
    type: 'regex',
  },
  {
    id: 'deny-chmod-recursive-system',
    pattern: 'chmod\\s+(-[a-zA-Z]*R[a-zA-Z]*|--recursive)(?:\\s+\\S+)*\\s+/(etc|var|usr|bin|sbin|lib)',
    reason: 'Destructive: recursive chmod on system dirs',
    type: 'regex',
  },

  // chown - dangerous ownership changes
  {
    id: 'deny-chown-recursive-system',
    pattern: 'chown\\s+(-[a-zA-Z]*R[a-zA-Z]*|--recursive)(?:\\s+\\S+)*\\s+/(etc|var|usr|bin|sbin|lib)',
    reason: 'Destructive: recursive chown on system dirs',
    type: 'regex',
  },
  {
    id: 'deny-chown-root',
    pattern: 'chown\\s+(-[a-zA-Z]*R[a-zA-Z]*|--recursive)(?:\\s+\\S+)*\\s+/\\s*$',
    reason: 'Destructive: chown on root',
    type: 'regex',
  },
  ...
  // Use [^|]* instead of .* before pipe to prevent backtracking
  {
    ...
    pattern: 'curl[^|]*\\|\\s*(bash|sh|zsh|python|perl|ruby)',
    reason: 'Remote code execution via curl pipe',
    type: 'regex',
  },
  {
    id: 'deny-wget-pipe-bash',
    pattern: 'wget[^|]*\\|\\s*(bash|sh|zsh|python|perl|ruby)',
    reason: 'Remote code execution via wget pipe',
    type: 'regex',
  },
  {
    id: 'deny-curl-pipe-sudo',
    pattern: 'curl[^|]*\\|\\s*sudo',
    reason: 'Remote code execution with elevated privileges',
    type: 'regex',
  },
  ...
  {id: 'deny-init-6', pattern: 'init 6', reason: 'System reboot', type: 'contains'},
  {
    ...
    pattern: 'systemctl\\s+(disable|mask)\\s+(sshd|networking|network|firewalld|iptables)',
  ...
  {
    id: 'deny-sudo-rm-rf',
    pattern: 'sudo\\s+rm\\s+(-[a-zA-Z]*r[a-zA-Z]*|--recursive)',
    reason: 'Elevated destructive file removal',
    type: 'regex',
  },
]
 ...
 */
export function matchesAllowlist(command: string, context: LogContext): ExternalRule | null {
 ...
 * Returns the matching rule if found, null otherwise.
 */
export function matchesDenylist(command: string, context: LogContext): ExternalRule | null {
