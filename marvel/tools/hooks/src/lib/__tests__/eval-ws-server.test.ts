...
describe('EvalWsServer', () => {
  ...
  describe('connection handling', () => {
    ...
    it('rejects second connection', async () => {
      ...
      const closedPromise = new Promise<void>(resolve => ws2.on('close', resolve))
      await closedPromise
  ...
  describe('evaluate', () => {
    it('runs a full evaluation cycle', async () => {
      ...
      const evalPromise = server.evaluate('Test prompt', {type: 'object', properties: {}, required: []}, 5000)
      ...
      await new Promise(resolve => setTimeout(resolve, 100))
      const userMsg = messages.find(m => (m as Record<string, unknown>).type === 'user') as Record<string, unknown>
    ...
    it('handles subsequent evaluations (prompt cache path)', async () => {
      ...
      const userMsgs = messages.filter(m => (m as Record<string, unknown>).type === 'user')
      const initMsgs = messages.filter(m => (m as Record<string, unknown>).type === 'control_request')
    ...
    it('handles can_use_tool permission request for read-only tools', async () => {
      ...
      const responses = messages.filter(m => (m as Record<string, unknown>).type === 'control_response')
