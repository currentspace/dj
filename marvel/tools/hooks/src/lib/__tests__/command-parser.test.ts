...
describe('splitCompoundCommand', () => {
  ...
  it('handles mixed operators', () => {
    expect(splitCompoundCommand('cd /tmp && ls; echo done || true')).toEqual(['cd /tmp', 'ls', 'echo done', 'true'])
  })
...
describe('toProjectRelativePath', () => {
  it('converts absolute path under project root to relative', () => {
    expect(toProjectRelativePath('/Users/foo/project/backend/src/file.ts', '/Users/foo/project')).toBe(
      'backend/src/file.ts',
    )
  })

  it('returns original path if outside project root', () => {
    expect(toProjectRelativePath('/other/path/file.ts', '/Users/foo/project')).toBe('/other/path/file.ts')
  })

  it('handles project root with trailing slash', () => {
    expect(toProjectRelativePath('/Users/foo/project/src/index.ts', '/Users/foo/project/')).toBe('src/index.ts')
  })

  it('returns original path when no project root available', () => {
    expect(toProjectRelativePath('/abs/path/file.ts', undefined)).toBe('/abs/path/file.ts')
  })
})
