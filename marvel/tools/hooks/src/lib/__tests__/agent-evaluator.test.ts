...
function createMockServer(overrides: Partial<MockEvalServer> = {}): MockEvalServer {
  ...
  mockEvalWsServer.mockImplementation(function () {
    return mockServer
  })
  return mockServer
}
...
describe('analyzeWithAgent', () => {
  ...
  it('includes suggested_rule in result when present', async () => {
    ...
    expect(result.suggestions?.allow).toEqual([{pattern: 'git status', reason: 'Read-only git command'}])
...
describe('session resume', () => {
  it('passes --resume flag after successful evaluation', async () => {
    ...
    const resumeCall = spawnCalls.find((call: unknown[]) => (call[1] as string[]).includes('--resume'))
