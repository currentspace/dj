...
describe('calculateRelevance', () => {
  describe('excludes_paths', () => {
    it('returns 0 when file matches excludes_paths', () => {
      ...
      const score = calculateRelevance(pack, '/project/marvel/tools/hooks/src/lib/relevance.ts', [])
      expect(score).toBe(0)
    })
    ...
    it('returns 0 for node_modules exclusion', () => {
      ...
      const score = calculateRelevance(pack, '/project/node_modules/@tanstack/react-query/index.ts', [])
      expect(score).toBe(0)
    })
    ...
    it('scores normally when file does not match excludes_paths', () => {
      ...
      const score = calculateRelevance(pack, '/project/src/app/page.tsx', [])
    ...
    it('scores normally when excludes_paths is empty', () => {
      ...
      const score = calculateRelevance(pack, '/project/marvel/tools/hooks/src/lib/foo.ts', [])
  ...
  describe('scoring signals', () => {
    ...
    it('scores sensitive_path match + extension as 25', () => {
      ...
      const score = calculateRelevance(pack, '/project/src/app/dashboard/page.tsx', [])
...
describe('selectTopPacks', () => {
  ...
  it('returns max 4 packs sorted by score', () => {
    const packs = Array.from({length: 6}, (_, i) => makePack({name: `pack-${i}`, references: {code_paths: ['src/']}}))
