...
import {SECURITY_DECISION_SCHEMA, META_EVALUATION_SCHEMA, isValidSecurityDecision} from '../evaluation-schemas.js'

describe('SECURITY_DECISION_SCHEMA', () => {
  it('has required top-level structure', () => {
    expect(SECURITY_DECISION_SCHEMA.type).toBe('object')
    expect(SECURITY_DECISION_SCHEMA.required).toEqual(['decision', 'reasoning', 'confidence'])
  })

  it('defines decision as enum with correct values', () => {
    const props = SECURITY_DECISION_SCHEMA.properties as Record<string, Record<string, unknown>>
