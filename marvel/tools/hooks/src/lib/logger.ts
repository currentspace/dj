...
const DEBUG_ENABLED = process.env.MARVEL_DEBUG === '1' || process.env.MARVEL_DEBUG === 'true'
...
export function buildHookContext(
  ...
  const toolInput =
    rawToolInput && typeof rawToolInput === 'object' ? (rawToolInput as Record<string, unknown>) : undefined
  ...
  const sessionId =
    (typeof input?.session_id === 'string' ? input.session_id : undefined) || process.env.MARVEL_SESSION_ID
...
function extractToolString(toolInput: Record<string, unknown> | undefined, key: string): string | undefined {
  if (!toolInput) {
    return undefined
  }
...
function writeLog(entry: LogEntry): void {
  ...
  if (logPath) {
    try {
      ...
      process.stderr.write(`[marvel-logger] Failed to write log file: ${message}\n`)
    }
  }
