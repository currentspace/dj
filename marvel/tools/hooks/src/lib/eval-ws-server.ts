...
export interface IEvalWsServer {
  close(): void
  evaluate(prompt: string, jsonSchema: Record<string, unknown>, timeoutMs: number): Promise<EvalSessionResult>
...
export class EvalWsServer implements IEvalWsServer {
   ...
   */
  async evaluate(prompt: string, jsonSchema: Record<string, unknown>, timeoutMs: number): Promise<EvalSessionResult> {
    this.resetIdleTimer()
  ...
  private handleResult(msg: NdjsonMessage): void {
    ...
    if (msg.is_error || msg.subtype?.startsWith('error_')) {
      pending.reject(new Error(`Evaluation error: ${msg.subtype || 'unknown'} — ${msg.result || ''}`))
      return
    }
   ...
   */
  private async sendInitialize(jsonSchema: Record<string, unknown>, timeoutMs: number): Promise<void> {
   ...
   */
  private sendUserMessage(prompt: string, timeoutMs: number): Promise<EvalSessionResult> {
    if (!this.ws) {
