...
function validateHookHandler(
  ...
  if (!isHookHandlerType(h.type)) {
    errors.push(`${hPrefix}.type: "${h.type}" is not valid. Must be: ${VALID_HANDLER_TYPES.join(', ')}`)
    return
  }
 ...
 */
function validateHooks(hooks: Record<string, unknown>, errors: string[], warnings: string[]): void {
  ...
  if (g.matcher !== undefined) {
    if (typeof g.matcher !== 'string') {
      ...
      if (typeof g.matcher === 'object' && g.matcher !== null) {
        ...
        errors.push(`${prefix}.matcher: Expected string (regex pattern), but got ${typeof g.matcher}`)
      }
    } else if (g.matcher === '') {
      warnings.push(`${prefix}.matcher: Empty string matches all. Consider omitting matcher instead.`)
    }

    // Warn if matcher is set for events that don't support it
    if (isMatcherlessEvent(event)) {
      warnings.push(`${prefix}.matcher: "${event}" does not support matchers. Matcher will be ignored.`)
    }
  }
