...
function analyzeEvaluations(evaluations: AgentEvaluation[], suggestions: Suggestion[]): AnalysisResult {
...
function formatReport(analysis: AnalysisResult): string {
  ...
  if (analysis.proposedAllowlist.length > 0) {
    ...
    for (const rule of analysis.proposedAllowlist) {
      ...
      lines.push(
        `  ${rule.count.toString().padStart(3)}x | conf=${rule.avgConfidence.toFixed(2)} | ${rule.pattern}${badge}`,
      )
  ...
  if (analysis.patterns.length > 0) {
    ...
    for (const p of analysis.patterns) {
      const decisionStr = Object.entries(p.decisions)
        ...
        .join(', ')
      lines.push(
        `  ${p.count.toString().padStart(3)}x | conf=${p.avgConfidence.toFixed(2)} | ${p.prefix.padEnd(20)} | ${decisionStr}`,
      )
    }
    lines.push('')
  }
