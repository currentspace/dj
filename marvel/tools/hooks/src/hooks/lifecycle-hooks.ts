...
export async function handleNotification(input: NotificationHookInput): Promise<SyncHookJSONOutput> {
  ...
  logActivity(
    'notification',
    {message: input.message, notification_type: input.notification_type, title: input.title},
    context,
  )
  return {}
}
...
function logActivity(activityType: ActivityEventType, data: Record<string, unknown>, context: LogContext): void {
