...
describe('handleUserPromptSubmit - status detection', () => {
  ...
  it('does NOT trigger for embedded status text', async () => {
    const result = await handleUserPromptSubmit(makeInput('I need marvel status report for the client'))
  ...
  it('returns {} for normal prompts', async () => {
    const result = await handleUserPromptSubmit(makeInput('Please add a new component'))
    expect(result).toEqual({})
  })
})
