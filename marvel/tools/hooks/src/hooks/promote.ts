...
export async function handlePromote(input: Record<string, unknown>): Promise<SyncHookJSONOutput> {
  ...
  if (report.security.candidates.length > 0) {
    ...
    lines.push(``)
    lines.push(`Found ${report.security.candidates.length} learned rules ready for promotion to allowlist.json:`)
    lines.push(``)
  ...
  const hasCandidates = report.security.candidates.length > 0 || report.domain.candidates.length > 0
