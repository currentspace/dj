...
export async function handlePreToolUse(input: PreToolUseHookInput): Promise<SyncHookJSONOutput> {
  ...
  if (toolName === 'Bash' && toolInput?.command) {
    ...
    const MERGE_PATTERNS = [/^gh\s+pr\s+merge\b/, /^gh\s+api\s+.*\/merge\b/, /^gh\s+pr\s+create\s+.*--merge\b/]
  ...
  if (runDir) {
    ...
    if (runState) {
      ...
      runState.recentActivity.push({
        data: {
          file: filePath,
          lessonCount: selectedPacks.reduce((sum, p) => sum + Math.min(p.lessons.length, MAX_LESSONS_PER_PACK), 0),
      ...
      runState.lastInjection = {
        file: filePath,
        lessons: selectedPacks.flatMap(p => p.lessons.slice(0, MAX_LESSONS_PER_PACK).map(l => l.title)),
        packs: selectedPacks.map(p => p.metadata.name),
    ...
    const injectionRecord: InjectionRecord = {
      file: filePath,
      lessons_injected: selectedPacks.flatMap(p => p.lessons.slice(0, MAX_LESSONS_PER_PACK).map(l => l.title)),
      packs_injected: selectedPacks.map(p => p.metadata.name),
      timestamp: new Date().toISOString(),
    }
    safeAppendFile(path.join(runDir, 'injections.jsonl'), JSON.stringify(injectionRecord) + '\n', logContext)
  }
...
function formatLessonsForContext(packs: LoadedPack[], filePath: string): string {
  ...
  for (const pack of packs) {
    ...
    const sorted = [...pack.lessons].sort((a, b) => (b.utility_score ?? 0.5) - (a.utility_score ?? 0.5))
...
function readRecentGuidance(runDir: string, context: LogContext, windowMinutes = 30): Guidance[] {
