...
import type {Guidance, InjectionRecord, Lesson, LessonOutcome, RunState, ToolCallRecord} from '../types.js'

import {safeAppendFile, safeParseJsonl, safeReadJson, safeWriteJson, safeWriteJsonl} from '../lib/file-ops.js'
import {buildHookContext, type LogContext, logDebug} from '../lib/logger.js'
import {findMarvelRoot, findRunDir} from '../lib/paths.js'
import {findDomainCandidates, findSecurityCandidates} from '../lib/promote.js'
 ...
 */
function archiveGuidance(guidance: Guidance[], marvelRoot: string, context: LogContext): void {
...
async function buildConcreteReflection(
  ...
  const totalCorrections = Object.values(categoryCounts).reduce((a, b) => a + b, 0)
  ...
  const domainCandidateLines: string[] = []
  if (hasCorrections) {
    const marvelRoot = findMarvelRoot()
    if (marvelRoot) {
  ...
  if (hasSecurity) {
    const patterns = security.candidates
      ...
      .join(', ')
    lines.push(`**Security:** ${security.candidates.length} rules (${patterns})`)
    lines.push('')
  }
 ...
 */
function correlateOutcomes(runDir: string, guidance: Guidance[], context: LogContext): void {
  const injections = safeParseJsonl<InjectionRecord>(path.join(runDir, 'injections.jsonl'), context)
  ...
  if (corrections.length === 0 && toolFailureFiles.size === 0) {
    ...
    const lessonCounts = new Map<string, {count: number; pack: string}>()
    for (const inj of injections) {
      for (let i = 0; i < inj.lessons_injected.length; i++) {
        ...
        if (existing) {
          ...
          lessonCounts.set(title, {count: 1, pack})
        }
      }
    }
    for (const [title, {count, pack}] of lessonCounts) {
      outcomes.push({followed_by_correction: 0, injected: count, lesson_title: title, pack})
    }
  ...
  const lessonStats = new Map<string, {corrected: number; injected: number; pack: string}>()
  for (const inj of injections) {
    const hadExplicitCorrection =
      correctionFiles.has(inj.file) || inj.packs_injected.some(p => correctionCategories.has(p))
  ...
  safeAppendFile(outcomesPath, lines, context)
  logDebug(
    `Wrote ${outcomes.length} lesson outcomes (${corrections.length} corrections, ${toolFailureFiles.size} tool failure files)`,
    context,
  )
}

function countCorrectionsByCategory(guidance: Guidance[]): Record<string, number> {
 ...
 */
function getToolFailureFiles(runDir: string, injections: InjectionRecord[], context: LogContext): Set<string> {
  const toolCalls = safeParseJsonl<ToolCallRecord>(path.join(runDir, 'tool_calls.jsonl'), context)
 ...
 */
function updateLessonUtilityScores(runDir: string, marvelRoot: string, context: LogContext): void {
  const outcomesPath = path.join(runDir, 'lesson-outcomes.jsonl')
  ...
  for (const [packName, packOutcomes] of outcomesByPack) {
    ...
    for (const lesson of lessons) {
      ...
      const sessionUtility = outcome.injected > 0 ? 1 - outcome.followed_by_correction / outcome.injected : 1
