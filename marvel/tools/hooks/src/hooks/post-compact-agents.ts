...
export async function handlePostCompactAgents(input: BaseHookInput): Promise<SyncHookJSONOutput> {
...
function buildContextInjection(agents: AgentEntry[], teamName: null | string): string {
  ...
  if (runningAgents.length > 0) {
    lines.push('')
    lines.push(
      `**Action Required:** ${runningAgents.length} agent(s) still running. Use TaskOutput with the agent ID to check results before proceeding.`,
    )
  }
